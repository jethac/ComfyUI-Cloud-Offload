...
PORTABLE_COMFY_TYPES = frozenset(
    {
        "IMAGE",
        "MASK",
        "LATENT",
...
class _Encoder:
    ...
    def encode(self, value: Any, depth: int = 0) -> dict[str, Any]:
        ...
        if (
            type(value).__name__ == "File3D"
            and callable(getattr(value, "get_bytes", None))
        ):
            name = f"blob-{len(self.blobs):08d}"
            self.blobs[name] = bytes(value.get_bytes())
            return {
                "kind": "file_artifact",
                "artifact_type": "file_3d",
                "blob": name,
                "format": str(getattr(value, "format", "") or "glb").lower(),
            }
        try:
            import torch
        except ImportError:  # pragma: no cover - ComfyUI always supplies torch
            torch = None
...
def _decode(node: dict[str, Any], tensors: dict[str, Any], blobs: dict[str, bytes], depth: int = 0) -> Any:
    if depth > MAX_TREE_DEPTH:
        raise PartitionProtocolError("Partition value nesting is too deep")
    kind = node.get("kind")
    if kind == "none":
    ...
    if kind == "file_artifact":
        ...
        if not file_format.isalnum() or len(file_format) > 16:
            raise PartitionProtocolError("Invalid file artifact format")
        result = {
            ARTIFACT_MARKER: artifact_type,
            "data": blobs[name],
            "format": file_format,
        }
        if "metadata" in node:
            result["metadata"] = _decode(node["metadata"], tensors, blobs, depth + 1)
        return result
...
def dump_bundle(value: Any, destination: str | Path) -> dict[str, Any]:
    ...
    if encoder.tensors:
        ...
        tensor_buffer.write(save(encoder.tensors))
    with zipfile.ZipFile(destination, "w", allowZip64=True) as archive:
        archive.writestr(MANIFEST_NAME, manifest, compress_type=zipfile.ZIP_DEFLATED)
        if encoder.tensors:
            archive.writestr(TENSORS_NAME, tensor_buffer.getvalue(), compress_type=zipfile.ZIP_STORED)
...
def load_bundle(source: str | Path, *, max_bytes: int | None = None) -> Any:
    ...
    with zipfile.ZipFile(source, "r") as archive:
        ...
        if manifest.get("schema") != SCHEMA:
            raise PartitionProtocolError(f"Unsupported partition schema: {manifest.get('schema')!r}")
